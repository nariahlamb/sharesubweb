...
function fetchFromBackend($pdo, $sids, $target, $userAgent) {
    ...
    $backendServers = ['http://localhost:25500/sub'];
    $backendUrl = $backendServers[array_rand($backendServers)] . "?target=" . urlencode($target) . "&url=" . $concatenatedUrls;

    return fetchContent($backendUrl, $userAgent);
}
...
function fetchContent($url, $userAgent) {
    ...
    if ($httpCode !== 200) {
        error_log("后端链接返回 HTTP 状态码: {$httpCode}");
        http_response_code(502);
        header('Content-Type: text/plain; charset=UTF-8');
        echo '';
        exit;
