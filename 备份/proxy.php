...
// 对 sid 的判断逻辑扩展：支持单个 SID、多 SID 和 all
if ($sid !== 'all') {
    // 检查是否为多个 SID（逗号分隔）
    if (strpos($sid, ',') === false) {
        // 单个 SID 的情况
...
try {
    ...
    if (strpos($sid, ',') !== false) {
        ...
        $cacheKey = "cache_sid_" . md5(implode('_', $sidArray) . $target);
        ...
        if (file_exists($cacheFile) && (time() - filemtime($cacheFile) < 7200)) {
            ...
            // 获取订阅信息
            $placeholders = implode(',', array_fill(0, count($sidArray), '?'));
            $stmt = $pdo->prepare("SELECT id, link FROM subscriptions WHERE id IN ($placeholders)");
            $stmt->execute($sidArray);
            ...
            $concatenatedUrls = implode('%7C', $encodedUrls); // %7C 是 URL 编码的 '|'

            // 构建后端请求 URL
            $backendServers = ['http://172.17.0.3:25500/sub'];
            ...
            curl_setopt($ch, CURLOPT_TIMEOUT, 1200); // 设置超时时间（秒）
        ...
        // 单个 SID 的情况（非 all，也非多个 SID）
        // 如果携带 &target= 参数且不为空，则执行转换逻辑，否则原有逻辑
        ...
        if (!empty($target)) {
            // 有 target 参数，则执行转换逻辑（与多 SID 类似）
            // 构建缓存键
            $cacheKey = "cache_sid_single_" . md5($sid . $target);
            $cacheFile = sys_get_temp_dir() . '/' . $cacheKey;
            ...
            if (file_exists($cacheFile) && (time() - filemtime($cacheFile) < 7200)) {
                ...
                // 构建后端请求 URL
                $encodedUrl = urlencode($subscription['link']);
                $backendServers = ['http://172.17.0.3:25500/sub'];
                $backendUrl = $backendServers[array_rand($backendServers)];
                $backendUrl .= "?target=" . urlencode($target) . "&url=" . $encodedUrl;
